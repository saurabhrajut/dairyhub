...
export default function ShiftReportCalc() {
  ...
  const handleWhatsAppShare = async () => {
    ...
    if (navigator.canShare && navigator.canShare({ files: [file] })) {
      ...
      await saveFile(blob, `Shift_Report_${date}.pdf`, "application/pdf");
      toast({
        title: "WhatsApp opened & PDF saved to device",
        description: "PDF report saved directly to your phone's Documents/Downloads. Please attach it if sharing.",
      });
    }
  };
  ...
  const handleEmailShare = async () => {
    ...
    if (navigator.canShare && navigator.canShare({ files: [file] })) {
      ...
      await saveFile(blob, `Shift_Report_${date}.pdf`, "application/pdf");
      toast({
        title: "Email opened & PDF saved to device",
        description: "PDF report saved directly to your phone's Documents/Downloads. Please attach it if sending.",
      });
    }
  };
  ...
  const handlePrint = () => {
    ...
    if (!printWindow) {
      toast({
        title: "Generating PDF for your Device...",
        description: "Direct print window is not supported on mobile. Generating PDF to save directly to your storage.",
      });
      (async () => {
        const blob = await generatePDFBlob();
        if (blob) {
          await saveFile(blob, `Shift_Report_${date}.pdf`, "application/pdf");
          toast({
            title: "PDF Saved to Device! 📄",
            description: `Saved Shift_Report_${date}.pdf directly to your Documents/Downloads folder.`,
          });
        }
      })();
      return;
