...
const LABELS = {
  hi: {
    ...
    topics: "Topics",
    modules: "Methods",
    langPill: "Chromatography Hindi Content",
  ...
  en: {
    ...
    topics: "Topics",
    modules: "Methods",
    langPill: "Chromatography English Content",
    topicsCount: (n: number) => `${n} topics`,
  },
};
...
// ─────────────────────────────────────────────
const TopicContent = ({ content, accent }: { content: any, accent: string }) => {
    ...
    <div className="w-full max-w-full overflow-hidden">
      {Object.keys(content).map((key) => {
        ...
        if (typeof section === 'object' && section !== null && section.title && section.content) {
          return (
            <Section key={key} title={section.title} accentColor={accent}>
              <div className="w-full max-w-full overflow-hidden break-words" dangerouslySetInnerHTML={{ __html: section.content }} />
            </Section>
          );
        }
        return null;
...
const getTopicGroups = (topicMap: any, lang: "hi" | "en") => {
  return [
    {
      groupLabel: lang === "hi" ? "उन्नत क्रोमैटोग्राफी (Advanced Chromatography)" : "Advanced Chromatography",
      groupIcon: Activity,
      topics: [
        {
          value: "hplc",
        ...
        },
        {
          ...
          subtitle: "Volatile & gaseous compound analysis",
          icon: Settings,
          accent: "purple",
          badge: "Advanced",
          badgeVariant: "secondary" as const,
    ...
    {
      groupLabel: lang === "hi" ? "मानक और पृथक्करण (Standard & Separation)" : "Standard & Separation",
      groupIcon: Layers,
      topics: [
        {
          value: "tlc",
          title: topicMap.tlc?.mainTitle || "Thin Layer (TLC)",
          subtitle: "Planar chromatography techniques",
          icon: Layers,
          accent: "amber",
          badge: "Standard",
          badgeVariant: "secondary" as const,
        ...
        },
        {
          ...
          subtitle: "Separation based on charge",
          icon: Droplet,
          accent: "emerald",
          badge: "Resins",
          badgeVariant: "secondary" as const,
        ...
        },
        {
          ...
          subtitle: "Molecular weight separation",
          icon: TestTube,
          accent: "orange",
          badge: "Filtration",
          badgeVariant: "secondary" as const,
    ...
    {
      groupLabel: lang === "hi" ? "परीक्षण के तरीके (Testing Methods)" : "Testing Methods",
      groupIcon: ListChecks,
      topics: [
        {
          value: "testing-methods",
          title: topicMap["testing-methods"]?.mainTitle || "Test Methods",
          subtitle: "Applications in dairy & food industry",
          icon: ListChecks,
          accent: "red",
          badge: "Application",
...
// ─────────────────────────────────────────────
export function ChromatographyModal({
  isOpen,
  setIsOpen,
}: {
  isOpen: boolean;
  ...
  const getDialogDescription = () => {
    if (selectedTopicData && selectedTopicData.description) return selectedTopicData.description;
    return lang === "hi" 
        ? "डेयरी उद्योग में उपयोग किए जाने वाले विभिन्न क्रोमैटोग्राफी विधियों का अन्वेषण करें।"
        : "Explore different chromatography methods used in the dairy industry.";
  };

  return (
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      {/* Global CSS for Strict HTML Boundary enforcement */}
      ...
      ">
        ...
        <div className="bg-gradient-to-br from-slate-900 via-sky-900 to-slate-800 px-3 sm:px-6 py-2 sm:py-4 shrink-0 border-b border-white/10">
          <DialogHeader>
            ...
            <DialogDescription className={`text-center text-sky-200/80 text-[10px] sm:text-sm line-clamp-1 px-2 mt-1 ${activeTopic ? "hidden sm:block" : "block"}`}>
              {selectedTopicInfo ? selectedTopicInfo.subtitle : getDialogDescription()}
            </DialogDescription>
          </DialogHeader>
          ...
          {!activeTopic && (
            <div className="hidden sm:flex flex-wrap justify-center gap-1.5 mt-2 sm:mt-3">
              ...
              <span className="inline-flex items-center gap-1 bg-white/10 text-white text-[10px] sm:text-xs px-2 sm:px-3 py-0.5 sm:py-1 rounded-full border border-white/20">
                <FlaskConical className="w-3 h-3 shrink-0" /> 3 {lbl.modules}
              </span>
        ...
        {/* ── Content Area ───────────────────────── */}
        {selectedTopicInfo && selectedTopicData ? (
          ...
          <div className="flex-1 min-h-0 overflow-y-auto overflow-x-auto bg-slate-50 strict-html-wrap">
            <div className="p-2 sm:p-4 md:p-6 max-w-5xl mx-auto w-full">
              <TopicContent content={selectedTopicData} accent={selectedTopicInfo.accent} />
            </div>
          </div>
