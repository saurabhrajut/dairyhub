...
// ─────────────────────────────────────────────────────────────────
// PRIMITIVE LAYOUT HELPERS (Preserved & Enhanced)
// ─────────────────────────────────────────────────────────────────
...
const Card = ({
  ...
  return (
    ...
    >
      ...
      <div className="space-y-3" style={{ maxWidth: "100%", wordBreak: "break-word", overflowWrap: "anywhere" }}>
        {children}
      </div>
    </div>
  );
...
const SectionHead = ({ title, icon: Icon }: { title: string; icon: React.ElementType }) => (
  <div className="flex items-center gap-3 mt-8 mb-5 pb-3 border-b border-slate-200">
    ...
    <h2 className="text-lg sm:text-xl font-bold text-slate-800 font-headline leading-tight" style={{ wordBreak: "break-word" }}>
      {title}
    </h2>
  </div>
);
...
const Td = ({
  ...
  <td
    className={`px-4 py-2.5 border-b border-slate-200 align-top text-xs leading-relaxed ${
...
const SimpleTable = ({
  ...
  <HScroll>
    <table className="w-full text-xs border-collapse bg-white" style={{ minWidth: minW }}>
      <thead>
      ...
      {d.whatIsMilk && (
        <Card title={d.whatIsMilk.title} accent="teal">
          ...
          {d.whatIsMilk.phases?.map((ph: string, i: number) => (
            <div key={i} className="pl-3 border-l-2 border-teal-400 mt-2" dangerouslySetInnerHTML={{ __html: ph }} />
          ))}
        </Card>
      )}
      ...
      {d.water && (
        <Card title={d.water.title} accent="slate">
          ...
          {d.water.forms?.map((form: any, i: number) => (
            <div key={i} className="mt-3 bg-white rounded-xl p-4 border border-slate-200 shadow-sm">
        ...
        <Card title={d.structure.title} accent="rose">
          ...
          {d.structure.structureLevels?.map((lvl: any, i: number) => (
            <div key={i} className="mt-2 flex gap-3 items-start bg-white p-3 rounded-xl border border-rose-100">
              <span className="mt-1 shrink-0 w-2 h-2 rounded-full bg-rose-400" />
              <p className="text-sm leading-relaxed text-slate-700" style={{ wordBreak: "break-word" }}>
      ...
      {d.physiology && (
        <Card title={d.physiology.title} accent="teal">
          ...
          {d.physiology.secretoryPathways?.map((path: any, i: number) => (
            <div key={i} className="mt-3 bg-white rounded-xl p-4 border-l-4 border-teal-400 shadow-sm">
          ...
          {d.physiology.ejectionTitle && (
            <div className="mt-5 bg-slate-900 rounded-xl p-5 border border-slate-800 shadow-md">
              ...
              <p className="text-sm leading-relaxed text-green-50" style={{ whiteSpace: "pre-wrap", wordBreak: "break-word" }}>
    ...
    <div className="space-y-4">
      ...
      {d.introduction && (
        <Card title={d.introduction.title} accent="emerald">
          ...
          {d.introduction.p3 && <P className="font-bold text-emerald-800">{d.introduction.p3}</P>}
        </Card>
      )}
      {d.composition && (
          ...
          return (
            <Card key={key} title={obj.title || key} accent="indigo">
              {Object.entries(obj).map(([k, v]) => {
                ...
                if (Array.isArray(v)) {
                  return (
                    <div key={k} className="mt-3 space-y-3">
        ...
        {activeTopic ? (
          <div className="flex-1 min-h-0 overflow-hidden bg-slate-50">
            <ScrollArea className="h-full w-full max-w-full">
              <div className="p-3 sm:p-5 md:p-8 max-w-5xl mx-auto w-full pb-10">
                {renderContent()}
              </div>
            </ScrollArea>
          </div>
