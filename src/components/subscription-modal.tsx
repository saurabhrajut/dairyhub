...
const departmentPlans: Record<string, Record<string, { title: string; price: number; duration: string; popular?: boolean }>> = {
    ...
    'all-control-access': {
        '7-days-ultimate': { title: "7 Days Ultimate", price: 89, duration: "All Access Pass" },
        '1-month-ultimate': { title: "1 Month Ultimate", price: 199, duration: "All Access Pass" },
        '6-months-ultimate': { title: "6 Months Ultimate", price: 499, duration: "Save 58%", popular: true },
        'yearly-ultimate': { title: "Yearly Ultimate", price: 999, duration: "Best Value" },
        'lifetime-ultimate': { title: "Lifetime Ultimate", price: 1499, duration: "One-Time Purchase" },
    },
...
export function SubscriptionModal({
  ...
  useEffect(() => {
    if(user?.department && user.department !== 'guest') {
  ...
  const handleSubscription = async (planKey: SubscriptionPlan, price: number) => {
    ...
    try {
        const orderResponse = await createRazorpayOrder(price);
        ...
        const options = {
            ...
            name: "Dairy Hub Pro",
            description: `Subscription for ${departmentPlans[selectedDept][planKey]?.title}`,
  ...
  const PlanCard = ({ planKey }: { planKey: string }) => {
    ...
    return (
        <div className={`border bg-white p-5 rounded-xl shadow-sm relative ${popular ? 'border-2 border-primary' : ''}`}>
            ...
            <Button onClick={() => handleSubscription(planKey as SubscriptionPlan, plan.price)} className={`w-full ${popular ? 'bg-gradient-to-r from-primary to-indigo-500 text-white shadow-md' : ''}`} variant={popular ? 'default' : 'outline'} disabled={!!isLoading}>
