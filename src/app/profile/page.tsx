...
export default function ProfilePage() {
  ...
  // ============================================================
  // 💳 RAZORPAY PAYMENT — BACKEND ORDER + FRONTEND CHECKOUT
  // ============================================================
  const handleRazorpayPayment = async () => {
    if (!selectedTier) {
      toast({ variant: 'destructive', title: 'Amount select karo', description: 'Pehle ek donation tier choose karo.' });
      return;
    }
    ...
    try {
      // Step 1: Backend se order_id lo (Key Secret safe rahega)
      const res = await fetch('/api/create-order', {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify({
          amount:  selectedTier.amount,
          receipt: `dairy_hub_${Date.now()}`,
          notes:   { userId: user?.uid || 'anonymous', userName: user?.displayName || '' },
        }),
      });
      ...
      if (!data.ok) {
        throw new Error(data.error || 'Order create nahi hua');
      }

      // Step 2: Razorpay script load karo
      const scriptLoaded = await loadRazorpayScript();
      if (!scriptLoaded) throw new Error('Payment gateway load nahi hua');
      ...
      const options = {
        ...
        // ✅ Sirf UPI methods dikhao — GPay, PhonePe, Paytm, etc.
        // Mobile number nahi mangega
        method: {
          upi:         true,   // GPay, PhonePe, Paytm, BHIM — sab
          card:        false,  // Card hide
          netbanking:  false,  // Netbanking hide
          wallet:      false,  // Wallet hide
          emi:         false,
          paylater:    false,
        },
          ...
          display: {
            blocks: {
              utib: {
                name: 'Pay via UPI',
                instruments: [
                  { method: 'upi', flows: ['intent'] }, // GPay, PhonePe, Paytm apps directly
                ],
              },
            },
            sequence: ['block.utib'],
        ...
        // Prefill mein sirf email — mobile nahi (mobile field remove)
        prefill: {
          name:    user?.displayName || '',
          email:   user?.email       || '',
          contact: '',  // Empty rakhne se mobile nahi mangega
        },
      ...
      setIsPaymentLoading(false);
      toast({ variant: 'destructive', title: 'Error', description: err.message || 'Kuch galat hua.' });
    }
  };
  ...
  return (
    ...
    <div className="max-w-md mx-auto min-h-screen relative overflow-hidden bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50">
      ...
      <div className="relative h-72 p-6 overflow-hidden select-none">
        ...
        <Link href="/" className="absolute top-6 left-6 z-20 glass-card p-2 rounded-full hover:bg-white/50 transition-all">
      ...
      <div className="px-6 pb-6 mt-6 space-y-4">
        ...
        <Dialog open={isDonationModalOpen} onOpenChange={(open) => {
          ...
          <div className="relative overflow-hidden p-6 rounded-2xl text-white shadow-xl bg-gradient-to-r from-pink-400 via-rose-400 to-red-400 neon-border">
            <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent" style={{ backgroundSize: '200% 100%', animation: 'shimmer 3s infinite' }}/>
          ...
          <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
            ...
            {paymentSuccess ? (
              <div className="py-8 text-center space-y-4">
                ...
                {lastPaymentId && (
                  <p className="text-xs text-gray-400 font-mono bg-gray-50 px-3 py-2 rounded-lg break-all">
