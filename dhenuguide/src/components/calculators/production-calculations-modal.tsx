...
const calculatorsInfo = {
    ...
    'plant-cost': { title: "Plant Costing", icon: DollarSign, component: PlantCostCalc },
};
...
function PlantCostCalc() {
  ...
  const FixedExpenseRow = memo(function FixedExpenseRow({ item, onChange, onRemove }: { item: FixedExpense, onChange: (id: number, field: keyof FixedExpense, value: string) => void, onRemove: (id: number) => void }) {
    return (
        <div className="grid grid-cols-1 sm:grid-cols-[2fr_1fr_auto] gap-2 items-center">
            <Input placeholder="Expense (e.g., Salaries)" value={item.name} onChange={(e) => onChange(item.id, 'name', e.target.value)} />
            <Input type="number" placeholder="Monthly Cost (₹)" value={item.cost} onChange={(e) => onChange(item.id, 'cost', e.target.value)} />
